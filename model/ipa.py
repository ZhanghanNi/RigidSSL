...
class VelocityHead(nn.Module):
    def __init__(self, c_s, c_hidden=None, num_layers=2):
        super(VelocityHead, self).__init__()

        self.c_s = c_s
        self.c_hidden = c_hidden if c_hidden is not None else c_s
        self.num_layers = num_layers

        trans_layers = []
        for i in range(num_layers):
            in_dim = self.c_s if i == 0 else self.c_hidden
            out_dim = 3 if i == num_layers - 1 else self.c_hidden
            init = "final" if i == num_layers - 1 else "relu"

            trans_layers.append(Linear(in_dim, out_dim, init=init))
            if i < num_layers - 1:
                trans_layers.append(nn.ReLU())
                trans_layers.append(nn.LayerNorm(out_dim))

        self.trans_mlp = nn.Sequential(*trans_layers)

        rot_layers = []
        for i in range(num_layers):
            in_dim = self.c_s if i == 0 else self.c_hidden
            out_dim = 4 if i == num_layers - 1 else self.c_hidden
            init = "final" if i == num_layers - 1 else "relu"

            rot_layers.append(Linear(in_dim, out_dim, init=init))
            if i < num_layers - 1:
                rot_layers.append(nn.ReLU())
                rot_layers.append(nn.LayerNorm(out_dim))

        self.rot_mlp = nn.Sequential(*rot_layers)

    def forward(self, node_embed):
        v_trans = self.trans_mlp(node_embed)
        v_rot = self.rot_mlp(node_embed)
        return v_trans, v_rot


class IpaVelocity(nn.Module):

    def __init__(self, model_conf):
        super(IpaVelocity, self).__init__()
        ...
        velocity_head_hidden = getattr(ipa_conf, 'velocity_head_hidden', ipa_conf.c_s)
        velocity_head_layers = getattr(ipa_conf, 'velocity_head_layers', 2)

        self.forward_velocity_head = VelocityHead(
            ipa_conf.c_s,
            c_hidden=velocity_head_hidden,
            num_layers=velocity_head_layers
        )
        self.backward_velocity_head = VelocityHead(
            ipa_conf.c_s,
            c_hidden=velocity_head_hidden,
            num_layers=velocity_head_layers
        )

    def forward(self, init_node_embed, edge_embed, input_feats, direction='forward'):
        ...
        if direction == 'forward':
            v_trans, v_rot = self.forward_velocity_head(node_embed)
        elif direction == 'backward':
            v_trans, v_rot = self.backward_velocity_head(node_embed)
        else:
            raise ValueError(f"Invalid direction: {direction}.")

        # mask not used
        v_trans = v_trans * diffuse_mask[..., None]
        v_rot = v_rot * diffuse_mask[..., None]
